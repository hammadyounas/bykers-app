...
export default function BuyBike() {
  ...
  return (
    ...
    <div className='flex max-md:flex-col w-full justify-between lg:pt-32 sm:pt-20 min-h-screen xl:px-20 lg:px-10 px-5'>
      <div className='mx-auto flex flex-col max-md:pt-20 max-lg:flex-col  lg:w-[40%] w-full'>
        ...
        {newBikeConstant.slice(0, 1).map((bike, index) => (
          <div key={index} className='mt-5 xl:w-[70%] md:w-[90%] w-full sm:mt-5 lg:text-base sm:text-sm text-xs mx-auto'>
      ...
      <div className="mt-5 grid grid-cols-2 ">
        <div className="col-span-2  w-full md:pr-4">
          <div className="flex justify-between border-b-2 py-2">
            ...
            <p className="max-lg:text-sm max-md:text-xs">{bike.title}</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
            <p className="text-xs text-gray-700 lg:text-sm">Price</p>
            <p className="max-lg:text-sm max-md:text-xs">{bike.price}</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
            <p className="text-xs text-gray-700 lg:text-sm">Model</p>
            <p className="max-lg:text-sm max-md:text-xs">{bike.model}</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
            <p className="text-xs text-gray-700 lg:text-sm">Engine</p>
            <p className="max-lg:text-sm max-md:text-xs">{bike.engine}</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
            <p className="text-xs text-gray-700 lg:text-sm">Displacement</p>
            <p className="max-lg:text-sm max-md:text-xs">{bike.displacement}</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
            <p className="text-xs text-gray-700 lg:text-sm">Horse Power</p>
            <p className="max-lg:text-sm max-md:text-xs">7.5 HP @ 8500.0 RPM</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
            <p className="text-xs text-gray-700 lg:text-sm">Petrol Capacity</p>
            <p className="max-lg:text-sm max-md:text-xs">{bike.petrolCapacity}</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
            <p className="text-xs text-gray-700 lg:text-sm">Fuel Average</p>
            <p className="max-lg:text-sm max-md:text-xs">{bike.fuelAverage}</p>
          </div>
          <div className="flex justify-between border-b-2 py-2">
      ...
      <div className="mt-5 w-[100%] sm:mt-10">
        <form className="-mx-4 flex  flex-wrap text-xs sm:text-sm lg:text-base xl:w-[90%]">
          {BuyBikesConstant.map(
            (item) => (
              <div className="mb-4 w-full px-4 xl:w-1/2 md:w-full sm:w-1/2" key={item.name}>
                {renderField(item)}
              </div>
            ),
          )}
