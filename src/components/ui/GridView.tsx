        ...
        <div className={`mt-10 w-full text-center ${isMenuOpen ? 'block' : 'hidden lg:block'}`}>
          {MenuItemsConstant.map((item, index) => (
            <Link href={item.link} key={index}>
              <div className={`w-full flex py-5 items-center justify-center border-b border-secondary-light cursor-pointer hover:bg-slate-100 hover:text-gray-800 ${router.pathname === item.link ? 'bg-secondary-light text-white' : ''}`}>
                <i className={`fa-solid ${item.icon} pr-4 text-gray-700 text-xl ${router.pathname === item.link ? ' text-white' : ''}` }></i>
